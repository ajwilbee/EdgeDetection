...
ImageSaveFolderName = 'OneRules_AllImages_MultipleIterations'; % make this ahead of time
...
for imNum = 1:length(ImageFiles)
   ...
   while ((low<ru)&(ru<upper))
        for k=1:16
            mkdir([ImageSaveFolderName '\' imFullName '\' num2str(ru)], 'JPG');
            mkdir([ImageSaveFolderName '\' imFullName '\' num2str(ru)], 'MAT');
            for i=1:n
                for j=1:m
                    if (j+2<=m) && (i+2<=n)
                            ...
                            mf = sum(abs(mask(2,2)-rule_nbd))/(sum( abs(mask(2,2)-rule_nbd))+t);
                            if mf >0.3
                                image(i+1,j+1)=1;
                            else
                                image(i+1,j+1)=0;
            ...
            saveas(figure(ru),strcat([ImageSaveFolderName '\' imFullName], '\JPG', '\Rule',num2str(ru),'Iteration',num2str(k),'.jpg'),'jpg');
            saveas(figure(ru),strcat([ImageSaveFolderName '\' imFullName], '\MAT', '\Rule',num2str(ru),'Iteration',num2str(k),'.fig'),'fig');
